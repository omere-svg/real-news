...
> An autonomous executive editor that reads 20+ official news & data APIs every few
> minutes and hands you the few stories that actually matter — each scored 0–10 by
> transparent math you can open and inspect, de-duplicated across time, and sized to
> the minutes you have — on a web app and a Telegram bot you log into with no password.
...
strip: **"20+ official APIs, zero scraping, updated N minutes ago"** — this is a live
...
it scannable. This is the moat vs every "LLM summarizes the news" entry.
...
just omitted for a lone source. Then the closer: *"This is built on 55+ ADRs and strict
TDD — run `npm test` yourself, the count is the proof. I ran QA cycles against the live
database that found and fixed a stored XSS, a data-loss race, and two sources silently
serving years-old data — each is a documented ADR. It engineers like a team, not a
hackathon."*

## Judge one-pager (hand out / slide)

**What it is:** an autonomous, server-side executive editor. A background worker pulls
20+ official public APIs every ~20 min, scores + de-duplicates + explains stories into a
local DB, served zero-latency on a web viewer and a Telegram bot.
...
2. **Engineering rigor** — 55+ ADRs (grill → architecture → TDD), a 701-test suite
   (`npm test` is the living count; 95.66% line / 85.45% branch coverage, CI-gated), and
   real production-DB QA cycles
...
**Numbers (verify live, don't trust this page):** 23 sources (17 active Story feeds + 6
numeric Signal sources; knesset-votes disabled upstream) · Significance 0.0–10.0 ·
tests: run `npm test` — the printed count (701 green at last edit) is the proof ·
ADRs: `ls docs/adr | wc -l` prints 56 — 55 numbered + the template · live accumulation counts:
`/api/stats` · ticks every ~20 min · deployed on an Oracle Cloud VM + Turso.
